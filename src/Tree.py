...
class Tree:
    ...
    def tree_from_array(self, array, predictions=None):
        """
        :return: Node
        """
    ...
    def _build_tree(self, arr, root, i, n):
        if i < n:
            temp = Node(arr[i]['id'], arr[i]['value'])
            root = temp
    ...
    def print_tree(self, predictions=None):
        """
        Print the tree
        """
        root = self.tree_from_array(np.concatenate([self.branch_nodes, self.leaf_nodes]), predictions)
        self._print_tree(root)

    def _print_tree(self, root, level=0, prefix="Root: "):
        if root is not None:
            print(" " * (level * 4) + prefix + f"({root.id}) " + str(root.value))
            if root.left or root.right:
                if root.left:
                    self._print_tree(root.left, level + 1, "L--- ")
                else:
                    print(" " * ((level + 1) * 4) + "L---" + f"({root.id})" + root.value)

                if root.right:
                    self._print_tree(root.right, level + 1, "R--- ")
                else:
                    print(" " * ((level + 1) * 4) + "R---" + f"({root.id})" + root.value)
