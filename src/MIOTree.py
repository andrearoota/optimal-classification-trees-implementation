...
class MIOTree:
    ...
    def __init__(self, alpha, min_samples_per_leaf, max_depth, X_train, y_train):
        self.alpha = alpha
    ...
    def tune(self, X_test, y_test):
        ...
        """
        C_max = np.power(2, self.max_depth) - 1
        warm_start_pool = []
        ...
        for D in range(2, self.max_depth + 1):
            for C in range(2, C_max + 1):
                ...
                node_map = {1:0}
                for t in new_model.pyomo_model.branch_nodes:
                    left = 2 * t
                    right = 2 * t + 1
                ...
                # create nodes
                nodes = {t: self.create_node(tree_from_model, node_map[t]) for t in new_model.pyomo_model.branch_nodes}
                nodes.update({t: self.create_node(tree_from_model, node_map[t], is_leaf=True) for t in new_model.pyomo_model.leaf_nodes})

                features_indices_len = len(new_model.pyomo_model.features_indices)

                # Set the branch nodes
                for t in new_model.tree.branch_nodes:
                    feature = nodes[t]["feature"]
                    threshold = nodes[t]["threshold"]

                    if feature is None or feature == -2: # model.tree_.TREE_UNDEFINED = -2
                        new_model.pyomo_model.d[t] = 0
                        new_model.pyomo_model.b[t] = threshold
                        for f in range(features_indices_len):
                            new_model.pyomo_model.a[f, t] = 0
                    else:
                        new_model.pyomo_model.d[t] = 1
                        new_model.pyomo_model.b[t] = 0
                        for f in range(features_indices_len):
                            if f == int(feature):
                                new_model.pyomo_model.a[f, t] = 1
                                new_model.pyomo_model.b[t] = threshold
                            else:
                                new_model.pyomo_model.a[f, t] = 0

                # Set the leaf nodes
                for t in new_model.tree.leaf_nodes:
                    value = nodes[t]["value"]
                    if value is None:
                        ...
                        if t % 2:
                            ...
                            for k in new_model.pyomo_model.classes_indices:
                                new_model.pyomo_model.c[k, t] = 0
                    else:
                        new_model.pyomo_model.l[t] = 1
                        for k in new_model.pyomo_model.classes_indices:
                            new_model.pyomo_model.c[k, t] = 1 if k == np.argmax(value) else 0

                accuracy = new_model.calculate_accuracy()
                print(f"CART\tDepth: {D}\tAlpha: {C}\tAccuracy: {accuracy}\tDuration: {duration}")

                # Search through pool of candidate warm starts (including CART) and choose the one with the lowest error
                new_warm_start_pool = np.concatenate((warm_start_pool, [{'model': new_model, 'accuracy': accuracy}]))
                best_model = max(new_warm_start_pool, key=lambda x: x['accuracy'])['model']
                ...
                print(f"MIO\tDepth: {D}\tAlpha: {C}\tAccuracy: {accuracy}\tDuration: {duration}")

                # Add the MIO solution to the warm start pool
                warm_start_pool = np.concatenate((warm_start_pool, [{'model': new_MIO_model, 'accuracy': accuracy}]))

        # Post-process the solution pool to remove all solutions that are not optimal to (24) for any value of α
        #best_model = max(warm_start_pool, key=lambda x: x['accuracy'])['model']
        ...
        best_result = max(warm_start_pool, key=lambda x: x['model'].calculate_accuracy(X_test, y_test))
    ...
    def create_model(self) -> pyo.ConcreteModel:
        ...
        model.M_constant = pyo.Param(initialize=M_constant, name="M_constant", doc="constant value")
        ...
        # add constraints
        def constraint_01(m, t):
            return sum(m.a[j, t] for j in m.features_indices) == m.d[t]
        model.c_01 = pyo.Constraint(branch_nodes, rule=constraint_01)

        def constraint_02(m, t): # always true (b[t] >= 0)
            return m.b[t] <= m.d[t] # and b[t] >= 0
        model.c_02 = pyo.Constraint(branch_nodes, rule=constraint_02)

        def constraint_03(m, t): # always true, is binary (0 or 1)
            return m.a[t] == 0 or m.a[t] == 1
        #model.c_03 = pyo.Constraint(branch_nodes, rule=constraint_03)

        def constraint_04(m, t):
            return m.d[t] <= m.d[t//2] # t//2 is the parent node
        model.c_04 = pyo.Constraint(branch_nodes[1:], rule=constraint_04)

        def constraint_05(m, i, t):
            return m.z[i, t] <= m.l[t]
        model.c_05 = pyo.Constraint(data_indices, leaf_nodes, rule=constraint_05)

        def constraint_06(m, t):
            return sum(m.z[i, t] for i in m.data_indices) >= m.min_samples_per_leaf * m.l[t]
        model.c_06 = pyo.Constraint(leaf_nodes, rule=constraint_06)

        def constraint_07(m, i):
            return sum(m.z[i, t] for t in m.leaf_nodes) == 1
        model.c_07 = pyo.Constraint(data_indices, rule=constraint_07)

        def constraint_08(_m, m, t, i): # check if this is correct
        ...
        model.c_08 = pyo.Constraint(data, rule=constraint_08)

        def constraint_09(_m, m, t, i): # check if this is correct
        ...
        model.c_09 = pyo.Constraint(data, rule=constraint_09)

        def constraint_10(m, t, k):
            return m.Nkt[k, t] == pyo.quicksum(m.z[i, t] * m.y_one_hot[i, k] for i in m.data_indices)                  
        model.c_10 = pyo.Constraint(leaf_nodes, classes_indices, rule=constraint_10)

        def constraint_11(m, t):
            return m.Nt[t] == sum(m.z[i, t] for i in m.data_indices)
        model.c_11 = pyo.Constraint(leaf_nodes, rule=constraint_11)

        def constraint_12(m, t):
            return sum(m.c[k, t] for k in m.classes_indices) == m.l[t]
        model.c_12 = pyo.Constraint(leaf_nodes, rule=constraint_12)

        def constraint_13(m, t, k):
            return m.L[t] >= m.Nt[t] - m.Nkt[k, t] - m.M_constant * (1 - m.c[k, t])
        model.c_13 = pyo.Constraint(leaf_nodes, classes_indices, rule=constraint_13)

        def constraint_14(m, t, k):
            return m.L[t] <= m.Nt[t] - m.Nkt[k, t] + m.M_constant * model.c[k, t]
        model.c_14 = pyo.Constraint(leaf_nodes, classes_indices, rule=constraint_14)

        def constraint_15(m, t):
            return m.L[t] >= 0
        model.c_15 = pyo.Constraint(leaf_nodes, rule=constraint_15)

        def constraint_16(m): # always true
            return m.C == m.d.sum()
        #model.c_16 = pyo.Constraint(rule=constraint_16)

        self.pyomo_model = model

    def create_node(self, tree_model, index, is_leaf=False):
        if index == -1:
            return {'feature': None, 'threshold': 0, 'value': None}
        return {
            'feature': None if is_leaf else tree_model.feature[index],
            'threshold': None if is_leaf else tree_model.threshold[index],
            'value': tree_model.value[index, 0] if is_leaf or index != -1 else None
        }
